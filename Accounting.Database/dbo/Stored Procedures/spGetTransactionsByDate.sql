...
BEGIN
		...
		where TransactionTimestamp=DATEADD(MONTH, DATEDIFF(MONTH, -1, GETDATE())-1, -1)
		...
		null as TransactionId, '...', 'None' as TransactionType
		from Transactions
		where TransactionTimestamp=DATEADD(MONTH, DATEDIFF(MONTH, -1, GETDATE())-1, -1)
		...
		AccountTypeId,Balance,TransactionId, AccountType, TransactionType
